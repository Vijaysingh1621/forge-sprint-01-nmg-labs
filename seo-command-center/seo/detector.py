...
def detect(rows: list[dict]) -> list[dict]:
    ...
    add("non_indexable_but_linked", "Medium",
        [r["Address"] for r in rows if (r.get("Indexability", "") or "").strip().lower() != "indexable" and _int(r.get("Inlinks")) > 0],
    ...
    add("slow_page", "Low",
        [r["Address"] for r in rows if is_html(r) and is_200(r) and _float(r.get("Response Time")) > 1.0],
...
def summarize(issues: list[dict]) -> dict:
    ...
    for i in issues:
        by_sev[i["severity"]] += 1
        # Calculate impact score: severity weight * number of affected URLs
        impact = weights.get(i["severity"], 1) * i["count"]
